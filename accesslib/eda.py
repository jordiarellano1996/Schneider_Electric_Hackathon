...
def get_time_code(df_in, col_to_merge, new_col_name):
    df_in[new_col_name] = pd.to_datetime(df_in[col_to_merge].apply(lambda x: "-".join(x.astype(str)), axis=1))
    return df_in
...
if __name__ == "__main__":
    ...
    columns_names = ['CITY ID', 'FacilityInspireID', 'DateTime', 'CONTINENT', 'City',
