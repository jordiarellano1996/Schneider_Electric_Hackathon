...
if __name__ == "__main__":
    ...
    df_test = pd.read_csv("/home/titoare/Documents/ds/seh/data/test/test_x.csv")
    ...
    df_test = get_time_code(df_test, ['reportingYear', 'MONTH', 'DAY'], "DateTime")
    ...
    # Test
    df_test['cityId_code'] = df_test["CITY ID"].astype('category').cat.codes
    df_test['FacilityInspireID_code'] = df_test["FacilityInspireID"].astype('category').cat.codes
    df_test['countryName_code'] = df_test["countryName"].astype('category').cat.codes
    df_test['City_code'] = df_test["City"].astype('category').cat.codes
    df_test['EPRTRAnnexIMainActivityCode_code'] = df_test["EPRTRAnnexIMainActivityCode"].astype('category').cat.codes
    df_test['eprtrSectorName_code'] = df_test["eprtrSectorName"].astype('category').cat.codes
    ...
    columns_names_test = ['CITY ID', "cityId_code", 'FacilityInspireID', "FacilityInspireID_code", 'DateTime',
                          'countryName', "countryName_code", 'CONTINENT', 'City', 'City_code',
                          'EPRTRAnnexIMainActivityCode', "EPRTRAnnexIMainActivityCode_code",
                          'EPRTRAnnexIMainActivityLabel',
                          'EPRTRSectorCode', 'eprtrSectorName', 'eprtrSectorName_code',
                          'avg_temp', 'avg_wind_speed', 'max_temp', 'max_wind_speed', 'min_temp', 'min_wind_speed',
                          'targetRelease', 'facilityName',
                          'reportingYear', 'MONTH', 'DAY',
                          'REPORTER NAME', 'DAY WITH FOGS', "test_index"]
    df = reindex_df(df, columns_names)
    df_test = reindex_df(df_test, columns_names_test)
